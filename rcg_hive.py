...
# ===============================> DROP TABLE EXISTS
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_BAD")
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_DUP01")
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_DUP02")
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_DUP03")
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_GOOD")
hv.sql("DROP TABLE IF EXISTS ooutput.hasilUSAGE_SA_RCG_REJECT")
hv.sql("DROP TABLE IF EXISTS output.hasilUSAGE_SA_RCG_SUM")
hv.sql("DROP TABLE IF EXISTS output.hasilMERGE_SA_RCG_GOOD")
hv.sql("DROP TABLE IF EXISTS output.hasilMERGE_SA_RCG_REJECT")
...
                .select('msisdn','subs_id','cust_type_desc','cust_subtype_desc')\
                .toDF('msisdn_sub','subs_id','cust_type_desc','cust_subtype_desc')
...
                .select('lac','ci','cell_name','vendor','site_id','ne_id','msc_name','node','longitude','latitude','region_network','provinsi','kabupaten','kecamatan','kelurahan','region_sales','branch','subbranch','address','cluster_sales','mitra_ad','cgi_pre','cgi_post','ocs_cluster','ocs_zone','area_sales','eci','lacci_id','file_date','file_id')\
...
                .withColumn('Flag',
                    when(trim(col('timestamp')).isNull() & trim(col('delta_balance')).isNull() & trim(col('msisdn')).isNull(),'BAD')
                     .otherwise('GOOD'))

# SPLIT FILTER (to check bad data)
df_TC_RCGbad = df_TC_RCG.filter(col('Flag') == 'BAD')
USAGE_OCS_RCG  = df_TC_RCG.filter(col('Flag')  == 'GOOD')\
                    .withColumn('filename', lit("RCG_JKT_Pre_20230222015959_00006238_51.1"))\
...
                    .withColumn('c1',F.concat(F.col('substr11_7_a_number_location'),F.lit('|'), F.col('substr18_3_a_number_location')))\
                    .withColumn('c2',F.concat(F.col('substr6_5_a_number_location'),F.lit('|'), F.col('substr11_5_a_number_location')))\
                    .withColumn('t1', F.regexp_replace('c1', r'^0+', ''))\
...
                            .otherwise(F.col('t2')))\
                    .select('timestamp_r','trx_hour','trx_date','msisdn','account','recharge_channel','expiration_date','serial_number','delta_balance','balance_amount','credit_indicator','recharge_method','recharge_id','bonus_information','provider_id','source_ip','user_id','result_code','bank_code','a_number_location','indicator_4g','balance_before','adjustment_reason','case_id','crmuser_id','old_expiration_date','split_code','recharge_amount','future_string_1','future_string_2','future_string_3','brand','file_id','rundate','load_user','site_name','timestamp_r','filename','rec_id','retry_count','rundate','status_data','status','trx_lacci')\
                    .toDF('timestamp_r','trx_hour','trx_date','msisdn','account','recharge_channel','expiration_date','serial_number','delta_balance','balance_amount','credit_indicator','recharge_method','recharge_id','bonus_information','provider_id','source_ip','user_id','result_code','bank_code','a_number_location','indicator_4g','balance_before','adjustment_reason','case_id','crmuser_id','old_expiration_date','split_code','recharge_amount','future_string_1','future_string_2','future_string_3','brand','file_id','load_ts','load_user','site_name','event_date','filename','rec_id','retry_count','retry_ts','status_data','status','trx_lacci')\
                    .join(df_SUB_DIM,col('msisdn') == col('msisdn_sub'), how = 'left')\
                    .withColumn('subs_id', 
...
                    .toDF('timestamp_r','trx_hour','trx_date','msisdn_sub','subs_id','cust_type_desc','cust_subtype_desc','account','recharge_channel','expiration_date','serial_number','delta_balance','balance_amount','credit_indicator','recharge_method','recharge_id','bonus_information','provider_id','source_ip','user_id','result_code','bank_code','a_number_location','indicator_4g','balance_before','adjustment_reason','case_id','crmuser_id','old_expiration_date','split_code','recharge_amount','future_string_1','future_string_2','future_string_3','brand','file_id','load_ts','load_user','site_name','event_date','filename','rec_id','retry_count','retry_ts','status_data','status','trx_lacci')\
...
# SPLIT FILTER
USAGE_OCS_RCG_01 = USAGE_OCS_RCG.filter(col('Flag')  == 'r1')
USAGE_OCS_RCG_02 = USAGE_OCS_RCG.filter(col('Flag')  == 'r2')
USAGE_OCS_RCG_03 = USAGE_OCS_RCG.filter(col('Flag')  == 'r3')
...
df_merge= reduce(DataFrame.unionAll, df)\
                .withColumn('flag_status', 
...
# SPLIT FILTER
df_DUP01 = df_merge.filter(col('flag_status')  == 'DUP01')
df_RCG_Part1 = df_merge.filter(col('flag_status')  == 'NODUP')\
...
                    .withColumn("r", dense_rank().over(window))\
                    .withColumn('status', 
...
                    .filter(col('recharge_method')  == 'TPFEE')
...
                        .join(df_SAProdline,col('substr_offer_id_3_5') == col('trim_note'), how = 'left')\
                        .withColumn('lacci_id', 
...
                        .toDF('timestamp_r','trx_hour','trx_date','msisdn','subs_id','cust_type_desc','cust_subtype_desc','account','recharge_channel','expiration_date','serial_number','delta_balance','balance_amount','credit_indicator','recharge_method','recharge_id','bonus_information','provider_id','source_ip','user_id','result_code','bank_code','a_number_location','lacci_id','lacci_closing_flag','lac','ci','node_type','area_sales','region_sales','branch','subbranch','cluster_sales','provinsi','kabupaten','kecamatan','kelurahan','indicator_4g','balance_before','adjustment_reason','case_id','crmuser_id','old_expiration_date','split_code','recharge_amount','future_string_1','future_string_2','future_string_3','brand','file_id','load_ts','load_user','site_name','event_date','filename','rec_id','retry_count','retry_ts','load_date','p_event_date')
...
# USAGE_SA_RCG_SUM
USAGE_SA_RCG_SUM = df_RCG_Part1.select('event_date','filename','status_data','status','recharge_amount')\
...
                 .groupby(['periode','filename','job_id','group_process','status_data','status','filetype']).agg(count('qty').alias("qty"), sum('recharge_amount').alias("rev"))\
